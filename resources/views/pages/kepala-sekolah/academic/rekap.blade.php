                ...
                @forelse($rows as $row)
                    <div class="p-4 space-y-3">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <a href="{{ route('kepala-sekolah.academic.student-detail', $row['student_id']) }}" class="font-bold text-slate-900 text-base hover:text-primary block truncate">{{ $row['name'] }}</a>
                                <p class="text-xs text-slate-500">NISN: {{ $row['nisn'] ?? '-' }} &bull; Kelas: {{ $row['class_name'] ?? '-' }}</p>
                            </div>
                        ...
                        <div class="flex items-center justify-end pt-2 border-t border-slate-100">
                            <a href="{{ route('kepala-sekolah.academic.student-detail', $row['student_id']) }}" class="inline-flex items-center justify-center px-3 py-2 text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg transition min-h-[44px]">
                                Detail Siswa &rarr;
                            </a>
                        </div>
                    </div>
                @empty
                    <div class="p-6 text-center text-sm text-slate-500">Belum ada data nilai untuk filter yang dipilih.</div>
                @endforelse
            </div>
