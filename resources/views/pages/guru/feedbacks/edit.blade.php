<x-layouts.app>
    <x-slot:title>Edit Feedback</x-slot:title>
    ...
    <div class="max-w-3xl space-y-6 mx-auto">
        <div>
            ...
            <x-page-header title="Edit Feedback" description="Perbarui umpan balik yang pernah diberikan." />
        </div>

        <x-card padding="lg">
            <form action="{{ route('guru.feedbacks.update', $feedback) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <x-select id="student_id" name="student_id" required>
                        ...
                        @foreach($students as $student)
                            <option value="{{ $student->id }}" {{ old('student_id', $feedback->student_id) == $student->id ? 'selected' : '' }}>{{ $student->user->name }}</option>
                        @endforeach
                    </x-select>
                ...
                <div>
                    ...
                    <x-select id="subject_id" name="subject_id">
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ old('subject_id', $feedback->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                        @endforeach
                    </x-select>
            ...
            <div>
                ...
                <x-text-input id="title" name="title" value="{{ old('title', $feedback->title) }}" required />
