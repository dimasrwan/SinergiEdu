<x-layouts.app>
    <x-slot:title>Detail Feedback: {{ $feedback->title }}</x-slot:title>

    <div class="w-full">
        <div class="mb-6">
            <a href="{{ route('guru.feedbacks.index') }}" class="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-slate-800 gap-1.5 transition">
        ...
        <x-card padding="none" class="overflow-hidden mb-6">
            <div class="bg-slate-50/50 p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex gap-2">
                    <x-button variant="secondary" as="a" href="{{ route('guru.feedbacks.edit', $feedback) }}" class="text-blue-600 hover:text-blue-700">
                        Edit
                    </x-button>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-6 mb-8 text-sm">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-bold text-slate-900">{{ $feedback->student->user->name ?? '-' }}</p>
                            <p class="text-xs text-slate-500">Siswa Penerima</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Message Bubble -->
                <div class="bg-slate-50 border border-slate-100 rounded-2xl p-6 text-slate-700 text-sm leading-relaxed whitespace-pre-wrap relative">
                    <!-- Decorative element for chat bubble style -->
                    <div class="absolute -top-3 left-8 w-6 h-6 bg-slate-50 border-t border-l border-slate-100 transform rotate-45"></div>
