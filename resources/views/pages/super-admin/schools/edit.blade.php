<x-layouts.app>
    <x-slot:title>Edit Profil Sekolah</x-slot:title>

    <div class="w-full max-w-5xl mx-auto">
        <div class="mb-8 flex flex-col items-start gap-4">
            <a href="{{ route('super_admin.schools.show', $school) }}" class="inline-flex items-center text-xs font-semibold text-slate-500 hover:text-slate-800 gap-1.5 transition">
            ...
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">Edit Profil Sekolah</h1>
                <p class="mt-2 text-sm text-slate-500">Perbarui informasi dan profil institusi SinergiEdu.</p>
            </div>
        </div>

        <x-card padding="none" class="overflow-hidden border border-slate-200/60 shadow-sm rounded-2xl bg-white">
            <form action="{{ route('super_admin.schools.update', $school) }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" :value="old('name', $school->name)" placeholder="Contoh: SMP Negeri 2 Banda Aceh" required class="w-full" />
                            ...
                            <div>
                                ...
                                <x-text-input id="npsn" name="npsn" type="text" :value="old('npsn', $school->npsn)" placeholder="Contoh: 10101010" class="w-full" />
                            ...
                            <div x-data="{ fileName: '{{ $school->logo ? 'Logo Saat Ini' : '' }}', filePreview: '{{ $school->logo ? Storage::url($school->logo) : '' }}' }">
                                        ...
                                        <template x-if="filePreview">
                                            <div class="flex items-center gap-4 w-full">
                                                ...
                                                <div class="flex-1 text-left min-w-0">
                                                    <p class="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-0.5" x-text="fileName === 'Logo Saat Ini' ? 'Logo Saat Ini' : 'File Terpilih'"></p>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="email" name="email" type="email" :value="old('email', $school->email)" placeholder="info@sekolah.sch.id" class="w-full" />
                            ...
                            <div>
                                ...
                                <x-text-input id="phone" name="phone" type="text" :value="old('phone', $school->phone)" placeholder="Contoh: 0651100200" class="w-full" />
                            ...
                            <div>
                                ...
                                <x-textarea id="address" name="address" rows="5" placeholder="Jl. Pendidikan No.2, Banda Aceh" class="w-full resize-y leading-relaxed">{{ old('address', $school->address) }}</x-textarea>
