<x-layouts.app title="Tambah Komite Sekolah">
    <div class="w-full space-y-6 pt-2 sm:pt-4">
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-sm max-w-3xl">
            <div class="border-b border-slate-100 pb-4 mb-6">
                <h1 class="text-xl sm:text-2xl font-bold text-slate-900">Tambah Komite Sekolah</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-1">Buat akun Komite Sekolah untuk {{ $school->name }}.</p>
            </div>

            <form action="{{ route('super_admin.schools.komite.store', $school) }}" method="POST" class="space-y-5">
                @csrf
