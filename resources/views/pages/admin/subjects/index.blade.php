<x-layouts.app>
    ...
    <div class="w-full">
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Manajemen Mata Pelajaran</h1>
            ...
            <div class="flex gap-2 w-full sm:w-auto shrink-0">
                <x-button variant="primary" href="{{ route('admin.subjects.create') }}" class="flex-1 sm:flex-none justify-center">
        ...
        <x-card padding="none" class="overflow-hidden">
            ...
            <div class="p-4 md:p-5 border-b border-slate-100 bg-slate-50/50">
                <form action="{{ route('admin.subjects.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1">
                    ...
                    <div class="flex gap-2 shrink-0">
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.subjects.index') }}" class="px-4 py-2 bg-white hover:bg-red-50 text-slate-500 hover:text-danger text-sm font-semibold rounded-xl border border-slate-200 transition-colors w-full sm:w-auto text-center">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($subjects as $index => $subject)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="py-4 px-6">
                                    <span class="inline-flex text-[11px] font-bold text-slate-600 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-md tracking-wide">
                                        {{ $subject->code }}
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    <p class="text-sm font-bold text-slate-900">{{ $subject->name }}</p>
                                </td>
                                <td class="py-4 px-6 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('admin.subjects.show', $subject) }}" class="p-1.5 text-slate-400 hover:text-accent hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                        </a>
                                        <a href="{{ route('admin.subjects.edit', $subject) }}" class="p-1.5 text-slate-400 hover:text-accent hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
