<x-layouts.app>
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex flex-col items-start gap-4 border-b border-slate-200 pb-5">
            <a href="{{ route('admin.semesters.index') }}" class="inline-flex items-center text-xs sm:text-sm font-semibold text-slate-500 hover:text-slate-800 gap-1.5 transition">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                </svg>
                Kembali ke Manajemen Semester
            </a>
            <div class="w-full flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Detail Semester {{ $semester->name }}</h1>
                    <p class="mt-1 text-xs sm:text-sm text-slate-500">Informasi rekap status semester dan volume penggunaannya dalam sistem.</p>
                </div>
        ...
        <x-card padding="lg" class="{{ $semester->is_active ? 'border-primary/20 ring-1 ring-primary/20' : '' }}">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-start">
                <!-- Info Utama (Left Column) -->
                <div class="space-y-6">
                    <div>
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Tahun Ajaran</p>
                        <h2 class="text-xl sm:text-2xl font-bold text-slate-900 leading-snug break-words">{{ $semester->academicYear->year }}</h2>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Semester</p>
                        <h2 class="text-3xl sm:text-4xl font-bold {{ $semester->is_active ? 'text-primary' : 'text-slate-900' }} leading-tight break-words">{{ $semester->name }}</h2>
                    </div>
                </div>
                
                <!-- Status & Metrik (Right Column) -->
                <div class="space-y-6 md:border-l md:border-slate-100 md:pl-8 lg:pl-12">
                    <div>
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Status Sistem</p>
                        @if($semester->is_active)
                            <div class="inline-flex items-center text-sm font-bold text-green-700 bg-green-100 px-3 py-1.5 rounded-lg border border-green-200">
                        ...
                        @else
                            <div class="inline-flex items-center text-sm font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
