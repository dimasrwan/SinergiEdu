<x-layouts.app>
    <x-slot:title>Manajemen Kepala Sekolah</x-slot:title>

    <div class="space-y-6">
        <!-- Page Header -->
        <div class="flex flex-col sm:flex-row sm:items-end justify-between border-b border-slate-200 pb-5 mb-6 gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Daftar Kepala Sekolah/Madrasah</h1>
                <p class="mt-2 text-sm text-slate-500 max-w-2xl">
                    Kelola data dan akun akses Kepala Sekolah/Madrasah.
                    <span class="inline-flex items-center ml-2 px-2 py-0.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-semibold border border-slate-200">
                        {{ $kepalaSekolah->total() }} Kepala Sekolah/Madrasah
                    </span>
                </p>
            </div>
            <div class="flex items-center gap-3 w-full sm:w-auto shrink-0">
                <x-button variant="primary" href="{{ route('admin.kepala-sekolah.create') }}">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
            <form action="{{ route('admin.kepala-sekolah.index') }}" method="GET" class="w-full md:w-96 relative">
        ...
        <div class="space-y-3 lg:hidden">
            @forelse($kepalaSekolah as $index => $item)
                @php
                ...
                <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <a href="{{ route('admin.kepala-sekolah.show', $item) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors shrink-0" title="Lihat Detail" aria-label="Lihat detail kepala sekolah">
                    ...
                    <div class="flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                        <a href="{{ route('admin.kepala-sekolah.edit', $item) }}" class="px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors min-h-[38px] inline-flex items-center gap-1.5">
                        ...
                        <button type="button" x-on:click.prevent="$dispatch('open-modal', 'delete-kepsek-{{ $item->id }}')" class="px-3 py-2 text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 rounded-xl transition-colors min-h-[38px] inline-flex items-center gap-1.5">
            ...
            @empty
                <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm text-center">
                    ...
                    @if(request('search'))
                        <h3 class="text-sm font-bold text-slate-900 mb-1">Kepala Sekolah/Madrasah tidak ditemukan</h3>
                        <p class="text-sm text-slate-500">Tidak ada Kepala Sekolah/Madrasah yang cocok dengan kata kunci pencarian Anda.</p>
                    @else
                        <h3 class="text-sm font-bold text-slate-900 mb-1">Belum ada Kepala Sekolah/Madrasah</h3>
                        <p class="text-sm text-slate-500 mb-4">Belum terdapat akun Kepala Sekolah/Madrasah yang terdaftar.</p>
                        <x-button variant="primary" href="{{ route('admin.kepala-sekolah.create') }}">
                            Tambah Kepala Sekolah/Madrasah
                        </x-button>
                    @endif
                </div>
            @endforelse

            @if($kepalaSekolah->hasPages())
                <div class="pt-2">
                    {{ $kepalaSekolah->links() }}
                </div>
            @endif
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('admin.kepala-sekolah.show', $item) }}" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail" aria-label="Lihat detail kepala sekolah">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                        </a>
                                        <a href="{{ route('admin.kepala-sekolah.edit', $item) }}" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit" aria-label="Edit kepala sekolah">
                                        ...
                                        <button type="button" x-on:click.prevent="$dispatch('open-modal', 'delete-kepsek-{{ $item->id }}')" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus" aria-label="Hapus kepala sekolah">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                                        </button>
                                    </div>
                                </td>
        ...
        @foreach($kepalaSekolah as $item)
            <x-modal name="delete-kepsek-{{ $item->id }}" maxWidth="sm">
                <div class="p-6">
                    ...
                    <p class="mt-2 text-sm text-slate-600">Apakah Anda yakin ingin menghapus data Kepala Sekolah/Madrasah <strong>{{ $item->user->name ?? '' }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
                    <div class="mt-6 flex justify-end gap-3">
                        <x-button variant="secondary" x-on:click="$dispatch('close-modal', 'delete-kepsek-{{ $item->id }}')">Batal</x-button>
                        <form action="{{ route('admin.kepala-sekolah.destroy', $item) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
