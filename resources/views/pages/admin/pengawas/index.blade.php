            ...
            @forelse($pengawas as $index => $item)
                ...
                <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    ...
                    <div class="flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                        ...
                        @else
                            <form action="{{ route('admin.pengawas.disconnect', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin melepas Pengawas ini dari sekolah Anda?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-3 py-2 text-xs font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-xl transition-colors min-h-[38px] inline-flex items-center gap-1.5">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @else
                                            <form action="{{ route('admin.pengawas.disconnect', $item) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin melepas Pengawas ini dari sekolah Anda?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Lepas dari Sekolah" aria-label="Lepas pengawas dari sekolah">
