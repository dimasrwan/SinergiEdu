<x-layouts.app>
    <x-slot:title>Manajemen Pengawas</x-slot:title>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between border-b border-slate-200 pb-5 mb-6 gap-4">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500 max-w-2xl">
                    ...
                    @else
                        Kelola Pengawas yang terhubung dengan sekolah Anda.
                    @endif
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto shrink-0">
                ...
                @else
                    <x-button variant="primary" href="{{ route('admin.pengawas.connect.form') }}">
                        <svg class="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        ...
        @endif
        @if(session('info'))
            <div class="p-4 bg-blue-50 border border-blue-100 rounded-2xl text-sm text-blue-800 flex items-center gap-3">
                <svg class="h-5 w-5 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.25v3.25a.75.75 0 001.5 0v-4A.75.75 0 0010 9H9z" clip-rule="evenodd" />
                </svg>
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
            <form action="{{ route('admin.pengawas.index') }}" method="GET" class="w-full md:w-96 relative">
        ...
        <div class="space-y-3 lg:hidden">
            @forelse($pengawas as $index => $item)
                ...
                <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    ...
                    <div class="flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                        ...
                        @else
                            <button type="button" x-on:click.prevent="$dispatch('open-modal', 'disconnect-pengawas-{{ $item->id }}')" class="px-3 py-2 text-xs font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-xl transition-colors min-h-[38px] inline-flex items-center gap-1.5">
            ...
            @empty
                <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm text-center">
                    ...
                    @else
                        <h3 class="text-sm font-bold text-slate-900 mb-1">Belum ada Pengawas Terhubung</h3>
                        <p class="text-sm text-slate-500 mb-4">Belum terdapat Pengawas yang terhubung dengan sekolah ini.</p>
                        @if($isSuperAdmin)
                            <x-button variant="primary" href="{{ route('admin.pengawas.create') }}">
                                Tambah Pengawas
                            </x-button>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @else
                                            <button type="button" x-on:click.prevent="$dispatch('open-modal', 'disconnect-pengawas-{{ $item->id }}')" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Lepas dari Sekolah" aria-label="Lepas pengawas dari sekolah">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244" /></svg>
                                            </button>
                                        @endif
                                    </div>
                                </td>
        ...
        <!-- Delete Modals Outside Loop (Super Admin Only) -->
        @if($isSuperAdmin)
            ...
            @endforeach
        @else
            <!-- Disconnect Modals Outside Loop (Admin Sekolah Only) -->
            @foreach($pengawas as $item)
                <x-modal name="disconnect-pengawas-{{ $item->id }}" maxWidth="sm">
                    <div class="p-6">
                        ...
                        <h2 class="text-lg font-bold text-slate-900">Lepas Pengawas dari Sekolah?</h2>
                        <p class="mt-2 text-sm text-slate-600">Pengawas <strong>{{ $item->user->name ?? '' }}</strong> akan dilepas dari sekolah Anda. Akun Pengawas dan penugasannya di sekolah lain tidak akan terpengaruh.</p>
                        <div class="mt-6 flex justify-end gap-3">
                            <x-button variant="secondary" x-on:click="$dispatch('close-modal', 'disconnect-pengawas-{{ $item->id }}')">Batal</x-button>
                            <form action="{{ route('admin.pengawas.disconnect', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <x-button variant="danger" type="submit" class="bg-amber-600 hover:bg-amber-700 text-white border-transparent">Lepas dari Sekolah</x-button>
                            </form>
                        </div>
                    </div>
                </x-modal>
