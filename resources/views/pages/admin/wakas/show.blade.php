<x-layouts.app>
    <x-slot:title>Detail Waka Kurikulum</x-slot:title>

    <div class="space-y-6">
        <!-- Page Header -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.wakas.index') }}" class="p-2 rounded-full hover:bg-slate-100 text-slate-500 hover:text-slate-700 transition">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                    </svg>
                </a>
                <div>
                    <h1 class="text-2xl font-bold tracking-tight text-slate-900">Profil Waka Kurikulum</h1>
                    <p class="text-sm text-slate-500">Detail informasi kepegawaian dan akun sistem.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <x-button variant="secondary" href="{{ route('admin.wakas.edit', $waka) }}">
                    Edit Profil
                </x-button>
            </div>
        </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                <x-card padding="none" class="overflow-hidden">
                    <div class="p-6 flex flex-col items-center text-center border-b border-slate-100">
                        ...
                        <h2 class="text-xl font-bold text-slate-900">{{ $waka->user->name ?? '-' }}</h2>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex items-start gap-2 text-sm text-slate-700 font-medium">
                                    ...
                                    <span>{{ $waka->address ?? 'Tidak tersedia' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <x-card padding="none">
                    <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                        <h2 class="text-lg font-bold text-slate-900">Informasi Waka</h2>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
                            <div>
                                <dt class="text-sm font-semibold text-slate-500">Nama Lengkap</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ $waka->user->name ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-semibold text-slate-500">NIP</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ $waka->nip ?? 'Belum Diatur' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-semibold text-slate-500">Email Login</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ $waka->user->email ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-semibold text-slate-500">Role Sistem</dt>
                                <dd class="mt-1 text-sm text-slate-900">Waka Kurikulum</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-sm font-semibold text-slate-500">Alamat Tempat Tinggal</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ $waka->address ?? 'Belum Diatur' }}</dd>
                            </div>
                        </dl>
                    </div>
                </x-card>
