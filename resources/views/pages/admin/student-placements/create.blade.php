<x-layouts.app>
    ...
    <div class="w-full max-w-2xl">
        ...
        <div class="mb-6 flex flex-col items-start gap-4">
            ...
            @endif
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Tambah Penempatan</h1>
                <p class="mt-1 text-sm text-slate-500">Tentukan siswa akan berada di kelas mana pada tahun ajaran yang dipilih.</p>
            </div>
        </div>
        ...
        <x-card padding="none" class="overflow-hidden">
            <form action="{{ route('admin.student-placements.store') }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            @if(request('student_id'))
                                ...
                                <select id="student_id" class="block w-full pl-3 pr-10 py-2 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent sm:text-sm" disabled>
                                    <option value="{{ $selectedStudent->id }}" selected>
                                        {{ $selectedStudent->user->name }} (NIS: {{ $selectedStudent->nis ?? '-' }})
                                    </option>
                                </select>
                            @else
                                @if($students->isEmpty())
                                    <p class="text-sm text-red-600 bg-red-50 p-2 rounded-lg border border-red-100">Belum ada data siswa di sistem.</p>
                                @else
                                    <select id="student_id" name="student_id" class="block w-full pl-3 pr-10 py-2 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent sm:text-sm" required>
                ...
                <div class="px-6 py-5 border-t border-slate-100 bg-slate-50/50 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 sm:gap-4">
                    @if(request('redirect_to') === 'student' && request('student_id'))
                        <x-button variant="secondary" href="{{ route('admin.students.show', request('student_id')) }}" class="w-full sm:w-auto">Batal</x-button>
                    @else
