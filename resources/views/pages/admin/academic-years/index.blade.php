<x-layouts.app>
    ...
    <div class="w-full">
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Manajemen Tahun Ajaran</h1>
            ...
            <div class="flex gap-2 w-full sm:w-auto shrink-0">
                <x-button variant="primary" href="{{ route('admin.academic-years.create') }}" class="flex-1 sm:flex-none justify-center">
            ...
            <div class="p-4 md:p-5 border-b border-slate-100 bg-slate-50/50">
                <form action="{{ route('admin.academic-years.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="flex gap-2 shrink-0">
                        ...
                        @if(request('status'))
                            <a href="{{ route('admin.academic-years.index') }}" class="px-4 py-2 bg-white hover:bg-red-50 text-slate-500 hover:text-danger text-sm font-semibold rounded-xl border border-slate-200 transition-colors w-full sm:w-auto text-center">
                                Reset
                            </a>
                        @endif
                    </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($academicYears as $index => $year)
                            <tr class="hover:bg-slate-50/50 transition-colors group {{ $year->is_active ? 'bg-blue-50/30' : '' }}">
                                ...
                                <td class="py-4 px-6">
                                    @if($year->is_active)
                                        <span class="inline-flex items-center text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-lg border border-green-200">AKTIF</span>
                                    @else
                                        <span class="inline-flex items-center text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200">TIDAK AKTIF</span>
                                    @endif
                                </td>
                                ...
                                <td class="py-4 px-6 text-right">
                                    <div class="flex items-center justify-end gap-2 ">
                                        ...
                                        <form action="{{ route('admin.academic-years.destroy', $year) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Tahun Ajaran ini?\n\nPENTING: Jika Tahun Ajaran ini sedang dipakai oleh Kelas atau Semester, sistem akan menolak penghapusan demi keamanan data.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg-lg transition-colors" title="Hapus" aria-label="Hapus tahun ajaran">
