<x-layouts.app>
            ...
            <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" x-data="teacherForm()" @submit="submitForm($event)">
                ...
                <div class="px-6 py-5 border-t border-slate-100 bg-slate-50/50 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 sm:gap-4">
                    ...
                    <button type="submit" :disabled="isSubmitting || hasDuplicate()" 
                        ...
                        <span x-text="isSubmitting ? 'Menyimpan...' : 'Perbarui Data Guru'"></span>
                    </button>
                </div>
            </form>
        </x-card>
    ...
    <script>
        function teacherForm() {
            // Restore previous inputs if available from validation errors, otherwise use existing assignments from DB
            const oldAssignments = @json(old('assignments', $existingAssignments ?? []));
