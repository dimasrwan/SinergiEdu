        ...
        <div class="bg-white border border-slate-200/60 rounded-3xl p-6 md:p-8 shadow-sm space-y-8">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-6">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.teacher-assignments.create', ['teacher_id' => $teacher->id, 'redirect_to' => 'teacher']) }}" class="px-4 py-2.5 text-sm font-semibold text-white bg-accent hover:bg-blue-600 rounded-xl transition duration-150 inline-flex items-center gap-1.5">
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td class="py-3 px-4 text-right">
                                            <div class="flex items-center justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                                <a href="{{ route('admin.teacher-assignments.edit', ['teacher_assignment' => $assignment->id, 'redirect_to' => 'teacher']) }}" class="p-1.5 text-slate-400 hover:text-accent hover:bg-blue-50 rounded-lg transition-colors" title="Edit Penugasan">
                                                ...
                                                <form action="{{ route('admin.teacher-assignments.destroy', $assignment) }}" method="POST" class="inline" onsubmit="return confirm('Hapus penugasan ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="redirect_to" value="teacher">
                                                    <button type="submit" class="p-1.5 text-slate-400 hover:text-danger hover:bg-red-50 rounded-lg transition-colors" title="Hapus Penugasan">
