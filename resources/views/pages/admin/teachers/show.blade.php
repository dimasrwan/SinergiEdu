<x-layouts.app>
    ...
    <div class="w-full space-y-6" x-data="{
        ...
        <div class="flex flex-col items-start gap-4 border-b border-slate-200 pb-5">
            ...
            <div class="w-full flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div class="flex items-center gap-3.5 sm:gap-4 min-w-0">
                    <div class="h-12 w-12 sm:h-14 sm:w-14 bg-blue-100 text-blue-700 font-bold rounded-2xl flex items-center justify-center text-xl sm:text-2xl shrink-0 shadow-2xs">
                        {{ strtoupper(substr($teacher->user->name ?? 'G', 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 leading-snug break-words">
                            {{ $teacher->user->name ?? '-' }}
                        </h1>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto shrink-0 pt-1 sm:pt-0">
                    <button type="button" x-on:click.prevent="$dispatch('open-modal', 'add-assignment')" class="w-full sm:w-auto px-4 py-2.5 text-xs sm:text-sm font-semibold text-white bg-accent hover:bg-blue-600 rounded-xl transition duration-150 inline-flex items-center justify-center gap-1.5 shadow-2xs">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                        Tambah Penugasan
                    </button>
                    <a href="{{ route('admin.teachers.edit', $teacher) }}" class="w-full sm:w-auto px-4 py-2.5 text-xs sm:text-sm font-semibold text-slate-700 hover:bg-slate-50 border border-slate-200 rounded-xl transition duration-150 inline-flex items-center justify-center shadow-2xs">
                        Edit Profil
                    </a>
                </div>
            </div>
        ...
        <!-- Main Detail Card -->
        <div class="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-2xs space-y-6 sm:space-y-8">
            ...
            <div class="space-y-4 pt-2">
                ...
                @if(session('success'))
                    <div class="p-3.5 sm:p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-start gap-3 shadow-2xs">
                ...
                @if(session('error'))
                    <div class="p-3.5 sm:p-4 bg-red-50 border border-red-100 rounded-xl flex items-start gap-3 shadow-2xs">
                ...
                <div class="block lg:hidden space-y-3">
                    @forelse($assignments ?? [] as $assignment)
                        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs space-y-2.5">
                                ...
                                <form action="{{ route('admin.teacher-assignments.update', $assignment) }}" method="POST">
                                    ...
                                    <div class="p-6 text-left whitespace-normal">
                                        ...
                                        <div class="mt-8 flex justify-end gap-3 pt-5 border-t border-slate-100">
                                            ...
                                            <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-primary rounded-lg hover:bg-blue-900 transition-colors">Simpan Perubahan</button>
                                        </div>
                                    </div>
                                </form>
                            </x-modal>
                    ...
                    <table class="w-full text-left border-collapse min-w-[768px] table-fixed">
                        <thead>
                            <tr class="bg-slate-50/80 border-b border-slate-200/80">
                                <th class="py-3.5 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider w-[28%]">Mata Pelajaran</th>
                                <th class="py-3.5 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider w-[14%]">Kelas</th>
                                <th class="py-3.5 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider w-[48%]">Periode</th>
                                <th class="py-3.5 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right w-[10%]">Aksi</th>
                            </tr>
                        </thead>
                            ...
                            @forelse($assignments ?? [] as $assignment)
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="py-3.5 px-6">
                                        <p class="text-sm font-bold text-slate-900 break-words leading-snug">{{ $assignment->subject->name }}</p>
                                    </td>
                                    <td class="py-3.5 px-6">
                                    ...
                                    <td class="py-3.5 px-6">
                                        <p class="text-sm text-slate-600 break-words leading-snug">
                                            {{ $assignment->academicYear?->year ?? '-' }} &middot; {{ $assignment->semester?->name ?? '-' }}
                                        </p>
                                    </td>
    ...
    <x-modal name="add-assignment" maxWidth="xl">
        <form action="{{ route('admin.teacher-assignments.store') }}" method="POST">
            ...
            <div class="p-6 text-left whitespace-normal">
                ...
                <div class="mt-8 flex justify-end gap-3 pt-5 border-t border-slate-100">
                    <button type="button" x-on:click.prevent="$dispatch('close-modal', 'add-assignment')" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">Batal</button>
