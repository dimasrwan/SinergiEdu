<x-layouts.app>
    <x-slot:title>Edit Komite Sekolah</x-slot:title>

    <div class="space-y-6 max-w-3xl mx-auto">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Edit Komite Sekolah</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-1">Perbarui data akun Komite Sekolah.</p>
            </div>
        ...
        <x-card padding="md">
            <form action="{{ route('admin.komite.update', $komite) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="password" name="password" id="password" class="w-full rounded-lg border-slate-200 text-sm focus:border-primary focus:ring-primary" placeholder="Biarkan kosong jika tidak diubah">
                        @error('password')
                            <p class="text-xs text-red-600 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
