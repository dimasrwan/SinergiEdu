                ...
                @forelse($materials as $material)
                    <div class="bg-white border border-slate-200/75 rounded-2xl p-4 sm:p-5 flex flex-col h-full shadow-sm hover:shadow-md hover:border-primary/40 transition group min-w-0">
                        ...
                        <div class="flex items-start justify-between mb-4 gap-2">
                            <span class="inline-flex text-[11px] font-bold text-primary uppercase tracking-wider min-w-0 break-words">
                        ...
                        <h3 class="text-base sm:text-[17px] font-bold text-slate-900 mb-2 leading-snug group-hover:text-primary transition-colors break-words min-w-0">{{ $material->title }}</h3>
                        
                        <div class="mt-auto space-y-4 pt-4 border-t border-slate-100 min-w-0">
                            <div class="flex items-center text-[13px] text-slate-500 font-medium min-w-0">
                                <span class="truncate min-w-0">{{ $material->teacher->user->name ?? '-' }}</span>
                                <span class="mx-2 shrink-0">&bull;</span>
                                <span class="shrink-0">{{ $material->created_at->format('d M Y') }}</span>
                            </div>
                            
                            <a href="{{ route('siswa.materials.show', $material->id) }}" class="flex items-center justify-center w-full py-2.5 min-h-[44px] bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-bold rounded-xl border border-slate-200 transition">
                                Buka Materi &rarr;
                            </a>
                        </div>
                    </div>
