<x-layouts.app>
    ...
    <div class="w-full space-y-6">
        ...
        <!-- Hero Minimalist -->
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100/50 rounded-2xl p-5 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm min-w-0">
            <div class="min-w-0">
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 mb-2 break-words">Hai, {{ auth()->user()->name }}! 👋</h1>
                @if(!$classroom)
                ...
                @else
                    ...
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-blue-100 text-xs font-bold text-primary shadow-sm min-w-0 break-words">
            ...
            @if($classroom && $upcomingAssignments->isNotEmpty())
                <div class="shrink-0">
                    <a href="{{ route('siswa.assignments.index') }}" class="inline-flex items-center justify-center min-h-[44px] px-6 py-2.5 bg-primary text-white hover:bg-primary/90 rounded-xl text-sm font-bold transition shadow-sm w-full md:w-auto">
                        Lihat Tugas &rarr;
                    </a>
                </div>
            @endif
