<x-layouts.app>
    <x-slot:title>Edit Jadwal Inspeksi - Pengawas</x-slot:title>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span>/</span>
            <a href="{{ route('pengawas.inspections.show', $inspection) }}" class="hover:text-slate-800 font-semibold transition">{{ $inspection->title }}</a>
            <span>/</span>
            <span class="text-slate-800 font-semibold">Edit</span>
        </div>

        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Jadwal Inspeksi</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui informasi jadwal inspeksi.</p>
        </div>

        <x-card padding="md">
            <form method="POST" action="{{ route('pengawas.inspections.update', $inspection) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $inspection->title) }}" required class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                        @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location', $inspection->location) }}" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                    @error('location')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
