<x-layouts.app>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <x-card padding="sm" class="min-w-0">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        <span class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 block truncate">Siswa</span>
                        <h3 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 truncate">{{ number_format($totalStudents) }}</h3>
                    </div>
                    <div class="text-blue-600 bg-blue-50/50 p-2.5 sm:p-3 rounded-xl border border-blue-100 shrink-0">
            ...
            <x-card padding="sm" class="min-w-0">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        <span class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 block truncate">Guru</span>
                        <h3 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 truncate">{{ number_format($totalTeachers) }}</h3>
                    </div>
                    <div class="text-emerald-600 bg-emerald-50/50 p-2.5 sm:p-3 rounded-xl border border-emerald-100 shrink-0">
                        <svg class="h-5 w-5 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="5"/><path d="M20 21a8 8 0 0 0-16 0"/></svg>
                    </div>
                </div>
            </x-card>
            ...
            <x-card padding="sm" class="min-w-0">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        <span class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 block truncate">Sekolah</span>
                        <h3 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 truncate">{{ number_format($totalSchools) }}</h3>
                    </div>
                    <div class="text-amber-600 bg-amber-50/50 p-2.5 sm:p-3 rounded-xl border border-amber-100 shrink-0">
            ...
            <x-card padding="sm" class="min-w-0">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        <span class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 block truncate">Rata-rata Nilai</span>
                        <h3 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 truncate">{{ $avgScore ? number_format($avgScore, 1) : '-' }}</h3>
                    </div>
                    <div class="text-blue-600 bg-blue-50/50 p-2.5 sm:p-3 rounded-xl border border-blue-100 shrink-0">
        ...
        <x-card padding="md">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-base sm:text-lg font-bold text-slate-900">Evaluasi Terakhir</h3>
                <a href="{{ route('pengawas.evaluations.index') }}" class="text-xs sm:text-sm text-primary hover:text-blue-700 font-semibold transition min-h-[44px] inline-flex items-center">Lihat Semua</a>
            </div>
                ...
                <div class="block sm:hidden divide-y divide-slate-100">
                    @foreach($evaluations as $eval)
                        <div class="py-3 space-y-1">
                            <h4 class="font-bold text-slate-900 text-sm truncate">{{ $eval->title }}</h4>
                            <div class="flex items-center justify-between text-xs text-slate-500">
                                <span>{{ $eval->created_at->format('d M Y, H:i') }}</span>
                                <a href="{{ route('pengawas.evaluations.edit', $eval) }}" class="text-primary font-semibold min-h-[44px] inline-flex items-center">Edit</a>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <x-card padding="md">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-base sm:text-lg font-bold text-slate-900">Jadwal Inspeksi Terakhir</h3>
                <a href="{{ route('pengawas.inspections.index') }}" class="text-xs sm:text-sm text-primary hover:text-blue-700 font-semibold transition min-h-[44px] inline-flex items-center">Lihat Semua</a>
            </div>
            @if($inspections->count())
                ...
                <div class="block sm:hidden divide-y divide-slate-100">
                    @foreach($inspections as $inspection)
                        <div class="py-3 space-y-1">
                            <h4 class="font-bold text-slate-900 text-sm truncate">{{ $inspection->title }}</h4>
                            <p class="text-xs text-slate-500">Sekolah: {{ $inspection->school?->name ?? '-' }}</p>
                            <div class="flex items-center justify-between text-xs text-slate-500">
                                <span>Tgl: {{ $inspection->inspection_date?->format('d M Y') ?? '-' }}</span>
                                <a href="{{ route('pengawas.inspections.show', $inspection) }}" class="text-primary font-semibold min-h-[44px] inline-flex items-center">Lihat</a>
                            </div>
                        </div>
                    @endforeach
                </div>
