            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label for="teacher_id" class="block text-sm font-semibold text-slate-700 mb-2">Sasaran Guru
                        (Opsional)</label>
                    <x-searchable-select id="teacher_id" name="teacher_id" placeholder="– Seluruh Sekolah / Bukan Guru Spesifik –" :selected="old('teacher_id')" :options="$teachers->map(fn($t) => ['value' => $t->id, 'label' => $t->user?->name])->toArray()" />
                    @error('teacher_id')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    <label for="class_id" class="block text-sm font-semibold text-slate-700 mb-2">Sasaran Kelas
                        (Opsional)</label>
                    <x-select id="class_id" name="class_id" placeholder="– Seluruh Sekolah / Bukan Kelas Spesifik –" :selected="old('class_id')" :options="$classrooms->map(fn($c) => ['value' => $c->id, 'label' => $c->name])->toArray()" />
                    @error('class_id')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
