<x-layouts.app>
    <x-slot:title>Edit Evaluasi</x-slot:title>
    ...
    <div class="w-full">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-slate-950 mt-4">Edit Evaluasi</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui catatan evaluasi sekolah Anda.</p>
        </div>

        <form action="{{ route('pengawas.evaluations.update', $evaluation) }}" method="POST" class="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="title" class="block text-sm font-semibold text-slate-700 mb-2">Judul Evaluasi</label>
                <x-text-input id="title" name="title" type="text" :value="old('title', $evaluation->title)" required placeholder="Contoh: Evaluasi Kinerja Akademik Semester Ganjil" />
                @error('title')
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="8" required placeholder="Tuliskan temuan pengawasan, masukan kurikulum, atau catatan perbaikan..."
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition duration-150">{{ old('content', $evaluation->content) }}</textarea>
                @error('content')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
