        ...
        <form action="{{ route('pengawas.action-plans.update', $actionPlan) }}" method="POST"
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="8" required
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-cyan-500 focus:bg-white transition duration-150 resize-none">{{ old('content', $actionPlan->content) }}</textarea>
                @error('content')
                <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
