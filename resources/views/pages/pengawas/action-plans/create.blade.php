<x-layouts.app>
    ...
    <div class="w-full">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-slate-950 mt-4">Buat Rencana Aksi Baru</h1>
            <p class="text-sm text-slate-500 mt-1">Rumuskan rencana aksi peningkatan mutu pembelajaran berdasarkan hasil
                supervisi.</p>
        </div>

        <form action="{{ route('pengawas.action-plans.store') }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <x-text-input id="title" name="title" type="text" :value="old('title')" required
                        placeholder="Contoh: Peningkatan Rata-rata Nilai Tes Awal Kelas X" class="w-full" />
                    @error('title')
                ...
                <div>
                    ...
                    <x-select id="class_id" name="class_id" placeholder="Seluruh Sekolah" :selected="old('class_id')" :options="$classrooms->map(fn($c) => ['value' => $c->id, 'label' => $c->name])->toArray()" />
                </div>

                <div>
                    <label for="academic_year_id" class="block text-sm font-semibold text-slate-700 mb-2">Tahun Ajaran</label>
                    <x-select id="academic_year_id" name="academic_year_id" placeholder="– Pilih Tahun Ajaran –" :selected="old('academic_year_id')" :options="$academicYears->map(fn($y) => ['value' => $y->id, 'label' => $y->name])->toArray()" />
                </div>

                <div>
                    <label for="priority" class="block text-sm font-semibold text-slate-700 mb-2">Prioritas <span class="text-red-500">*</span></label>
                    <x-select id="priority" name="priority" required :selected="old('priority', 'medium')" :options="[
                ...
                <div>
                    ...
                    <x-select id="status" name="status" required :selected="old('status', 'draft')" :options="[
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="8" required
                    placeholder="Tuliskan rencana aksi secara rinci: temuan, langkah perbaikan, target capaian, dan timeline pelaksanaan..."
