<x-layouts.app>
    ...
    <div class="w-full">
        <div class="mb-6">
            ...
            <x-page-header title="Edit Kelas" description="Ubah nama atau tingkat kelas yang terpilih." />
        </div>

        <x-card padding="lg">
            <form action="{{ route('waka.classes.update', $class) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="name" :value="__('Nama Kelas')" />
                    <x-text-input id="name" name="name" type="text" :value="old('name', $class->name)" placeholder="Contoh: XI-IPA-1" required />
                ...
                <div>
                    ...
                    <x-select id="grade_level" name="grade_level" required>
                        ...
                        <option value="10" {{ old('grade_level', $class->grade_level) == '10' ? 'selected' : '' }}>10 (Sepuluh)</option>
                        <option value="11" {{ old('grade_level', $class->grade_level) == '11' ? 'selected' : '' }}>11 (Sebelas)</option>
                        <option value="12" {{ old('grade_level', $class->grade_level) == '12' ? 'selected' : '' }}>12 (Duabelas)</option>
                    </x-select>
