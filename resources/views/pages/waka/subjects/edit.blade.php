<x-layouts.app>
    ...
    <div class="w-full">
        <div class="mb-6">
            ...
            <x-page-header title="Edit Mata Pelajaran" description="Ubah nama atau kode dari mata pelajaran terdaftar." />
        </div>

        <x-card padding="lg">
            <form action="{{ route('waka.subjects.update', $subject) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="name" :value="__('Nama Mata Pelajaran')" />
                    <x-text-input id="name" name="name" type="text" :value="old('name', $subject->name)" placeholder="Contoh: Matematika Wajib" required />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="code" :value="__('Kode Mata Pelajaran')" />
                    <x-text-input id="code" name="code" type="text" :value="old('code', $subject->code)" placeholder="Contoh: MTK-WJB" required />
